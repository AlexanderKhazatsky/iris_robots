...
- Todo's littered around (later)

"""


# Temporary Functions #
def get_images(num_images=6, coeff=1):
    return [np.random.randint(0, int(255 / ((i * 0.2 + 1) * coeff)), size=(256, 256, 3), dtype=np.uint8) 
        for i in range(num_images)]

def get_last_trajectory(num_images=6, horizon=50):
    return [get_images(num_images=num_images, coeff=i*0.05 + 1) for i in range(horizon)]

def collect_traj():
    time.sleep(10)
    return np.random.uniform() < 0.5

last_traj = get_last_trajectory()
...
class RobotGUI(tk.Tk):
    ...
    def set_img(self, i, widget=None, width=None, height=None):
        imgs = self.robot.get_camera_feed() #maybe do this better TODO
        if self.time_index is None: img = imgs[self.camera_order[i]]
        else: img = self.robot.traj_data['observations'][self.time_index][self.camera_order[i]]
    ...
    def update_time_index(self):
        if self.time_index is not None:
            self.time_index = (self.time_index + 1) % len(last_traj)
...
class CameraPage(tk.Frame):
    ...
    def update_camera_feed(self, i, w_coeff=0.75, h_coeff=1.):
        while True:
    ...
    def update_timer(self, seconds):
        ...
        curr_seconds = int(seconds) % 60

        if curr_seconds < 10: seconds_str = '0{0}'.format(curr_seconds)
        else: seconds_str = str(curr_seconds)

        is_robot_reset = self.controller.robot.traj_running
        if is_robot_reset or seconds != 0: seconds += 0.1

        self.time_str.set('{0}:{1}'.format(minutes_str, seconds_str))
        self.controller.after(100, lambda: self.update_timer(seconds))
